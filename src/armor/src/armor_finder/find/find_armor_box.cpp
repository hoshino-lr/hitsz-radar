...
// 在给定的图像上寻找装甲板
bool ArmorFinder::findArmorBox(const cv::Mat &src, std::vector<ArmorBox> &box) {
    ...
    for (auto & it:last_box)
    {
        ...
        sort(armor_boxes.begin(), armor_boxes.end(), [&](const ArmorBox &a, const ArmorBox &b) {
            if (it.rect != cv::Rect2d()) {
                return getPointLength(a.getCenter() - it.getCenter()) <
                       getPointLength(b.getCenter() - it.getCenter());
            } else {
                return a < b;
            }
        });
        box[sort_++] = armor_boxes[0];
    }
    for (auto & it:box)
    {
        if (it.rect != cv::Rect2d(0, 0, 0, 0)) {
            return true;
        }
    }
    return false;
}
cv::Point2d ArmorFinder::get_average(int num)
{
  cv::Point2d rtn;int efi_data = 0;
  for (int i = 0; i < this->points.at(num).filtering.size(); ++i) {
    if (this->points.at(num).filtering[i]!=cv::Point2d(0,0))
    {
      efi_data++;
    }
    rtn+=this->points.at(num).filtering[i];
  }
    return rtn/efi_data;
}
